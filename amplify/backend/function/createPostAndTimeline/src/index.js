...
exports.handler = async (event, context, callback) => {
  ...
  if (event.arguments.content.length > 140) {
    // do not allow post length greater than 140 characters.
  ...
  if (
    ...
    // for local mock
    env = {
      API_PRECISGQL_GRAPHQLAPIENDPOINTOUTPUT: 'http://localhost:20002/graphql',
      REGION: 'us-east-1',
    };
    graphql_auth = {
      type: 'AWS_IAM',
  ...
  if (!graphqlClient) {
    console.log({
      url: env.API_PRECISGQL_GRAPHQLAPIENDPOINTOUTPUT,
      region: env.REGION,
      auth: graphql_auth,
      disableOffline: true,
    });
